...
const EncargosProfesionalesC = ({ onBack }) => {
  ...
  // Cálculos
  const calcularBaseEncargo = (vr, monto) => {
  ...
  const handleCalcular = () => {
    ...
    if (tipoObra === '1') {
      const base = calcularBaseEncargo(vrNum, montoNum);
      ...
      <div className="form-card">
        ...
        {resultado && (
          <div className="resultado-card">
            {resultado.tipo === 'especiales' && (
              <div className="text-center">
                <h4 className="text-muted">{resultado.mensaje}</h4>
                <p className="text-muted">Esta funcionalidad estará disponible próximamente.</p>
              </div>
            )}

            {resultado.tipo === 'arquitectura' && (
              <>
